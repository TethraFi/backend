...
export class TapToTradeService {
  ...
  createOrder(params: CreateTapToTradeOrderRequest): TapToTradeOrder {
    ...
    const marketExecutor = chainConfig.contracts.tapToTradeExecutor;

    this.logger.info(`🔍 Creating order for ${chain} chain with contract: ${marketExecutor}`);
    ...
    const order: TapToTradeOrder = {
      ...
      chain: params.chain || 'base', // Default to base for backward compatibility
  ...
  /**
   * Update order status (generic method for monitor)
   */
  updateOrderStatus(
    orderId: string,
    ...
    if (updates) {
      if (updates.txHash) order.txHash = updates.txHash;
      if (updates.positionId) order.positionId = updates.positionId;
      if (updates.executionPrice) order.executionPrice = updates.executionPrice;
      if (updates.executedAt) order.executedAt = updates.executedAt * 1000; // Convert to ms
      if (updates.errorMessage) order.errorMessage = updates.errorMessage;

      if (status === TapToTradeOrderStatus.EXECUTED && !order.executedAt) {
        order.executedAt = Date.now();
      }
      if (status === TapToTradeOrderStatus.EXPIRED && !order.expiredAt) {
